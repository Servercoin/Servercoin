               ...
               if PROOFOFHAPPEN == True:
                   ...
                   if PROOFOFHAPPEN3 == True:
                            ...
                            while FINISHEDTHESTUFF4EVER == False:
                             ...
                             NEWBLOCKNUM
                             servers = {}
                             if blocklenthing == int(NEWBLOCKNUM):
                              try:
                              ...
                              serverthingthing.setblocknum(NEWBLOCKNUM)
                              try:
...
# Serialize the public key
                               ...
                               try:
                                ...
                                try:
                                 ...
                                 for item in trueserverlist["Data"]:
                                  ...
                                  urltosendto = str(trueserverlist["Data"][item])
                                 ...
                                 for item in datalistpower2:
                                   ...
                                   EASYTOUSEDATATHING[wallet] = {"Verifyingkey":verifyingkey,"Verifyingkeyspecial":load_pem_public_key(convertthething(str(verifyingkey)).encode('utf-8'),backend=default_backend)}
                              ...
                              if PROOFOFHAPPEN33 == True:
                                   ...
                                   for item in datalistpower2:
                                       Wallet = datalistpower2[item]["walletname"]
                                       Verifyingkey = datalistpower2[item]["verifyingkey"]
                                       SPECIALKEY = EASYTOUSEDATATHING[Wallet]["Verifyingkeyspecial"]
                                       Coins = WALLETVALUES[Wallet]["Coins"]
                                       COMBINETHEMBOTHFOREVERLOL2[Wallet] = {"Verifyingkeyspecial":SPECIALKEY
...
def loop1():
 the600thing = 0
 timeatstarttime = 0
 with open("timeatstart.txt","r") as file:
    try:
     timeatstarttime = float(file.read())
    except:
        lol = True
 totaltime = time.time()-timeatstarttime
 with open("changethe600thing.txt","r") as file:
    fileread = str(file.read())
 ...
 while True:
     while the600thing>0:
         ...
         DATA={}
         try:
          response23 = requests.post("http://"+str(ipthing)+":"+str(SPECIALPORT)+"/checkthe600thing",json=DATA)
          
          response23 = response23.json()
          response23 = response23["Success"]
     ...
     ipthing = get_local_ip2()
     try:
      requests.post("http://"+str(ipthing)+":"+str(SPECIALPORT)+"/gothroughtransactions")
     except:
         lol=True
     loadloop = True
     numthing=3
     while numthing>0.3:
      time.sleep(0.25)
      try:
       response222 = requests.get("http://"+str(ipthing)+":"+str(SPECIALPORT)+"/getthecountdownthing")
       if response222.status_code == 200:
        responsedata = response222.json()
        numthing = float(responsedata["Success"])
        if numthing<0.3:
          loadloop = False
          try:
           requests.post("http://"+str(ipthing)+":"+str(SPECIALPORT)+"/checkthecountdowthing")
          except:
              lol=True
          with open("changethe600thing.txt","r") as file:
              the600thing = float(file.read())
         ...
         try:
          response666 = requests.post("http://"+str(ipthing)+":"+str(SPECIALPORT)+"/checkthecountdowthing")
         except:
             lol=True
      except:
         lol=True
